    ...
    <table className="w-full min-w-[800px] text-left">
      <thead>
        <tr className="bg-stone-50/50">
          <th className="px-8 py-4 text-[10px] font-black uppercase tracking-widest text-stone-400">{sellerType === 'service' ? 'Job ID' : sellerType === 'both' ? 'Order / Job ID' : 'Order ID'}</th>
          <th className="px-8 py-4 text-[10px] font-black uppercase tracking-widest text-stone-400">Customer</th>
          <th className="px-8 py-4 text-[10px] font-black uppercase tracking-widest text-stone-400">{sellerType === 'service' ? 'Service' : sellerType === 'both' ? 'Item' : 'Product'}</th>
      ...
      <tbody className="divide-y divide-stone-50 bg-white">
        {filteredOrders.map((item) => {
          const isServiceItem = sellerType === 'both' ? (item.products?.stock ?? 0) >= 999 : sellerType === 'service'
          return (
            <tr key={item.id} className="group hover:bg-stone-50/30 transition-colors">
              ...
              <td className="px-8 py-6">
                <div className="text-sm font-bold text-slate-900 group-hover:text-stone-600 transition-colors">
                  {item.products?.title}
                  {sellerType === 'both' && (
                    <span className={`ml-2 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[8px] font-black uppercase tracking-wider ${
                      isServiceItem ? 'bg-violet-100 text-violet-700' : 'bg-blue-100 text-blue-700'
                    }`}>
                      {isServiceItem ? '⚡' : '📦'}
                    </span>
                  )}
                </div>
                {!isServiceItem && <div className="text-[10px] text-stone-400 font-medium mt-0.5">Quantity: {item.qty}</div>}
              </td>
              ...
              <td className="px-8 py-6">
                <Badge 
                  variant={
                    item.item_status === 'delivered' || item.item_status === 'completed' ? 'success' : 
